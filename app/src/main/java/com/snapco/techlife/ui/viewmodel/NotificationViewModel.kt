...
//        loadNotifications()
        ...
        fun getFormattedTimeDifference(notificationTime: String): String {
            val notificationDateTime =
                LocalDateTime.parse(notificationTime, DateTimeFormatter.ISO_DATE_TIME)
                    ...
                    if (response.isSuccessful) {
                        ...
                        if (responseBody?.success == true) {
                            ...
                            Log.d(
                                ...
                                "Notification added: ${responseBody.notification}"
                            )
                        } else {
                            Log.e(
                                "NotificationViewModel",
                                "Failed to add notification: ${responseBody?.message}"
                            )
                        }
                    } else {
                        Log.e(
        ...
        fun markAsSeen(notificationId: String) {
            viewModelScope.launch {
                try {
                    ...
                    val response =
                        ApiClient.apiService.updateNotificationStatus(notificationId, requestBody)
                    if (!response.isSuccessful) {
                        Log.e(
                            "NotificationViewModel",
                            "Failed to update notification status with code: ${response.code()}"
                        )
                    } else {
